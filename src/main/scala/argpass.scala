...
object argpass {
  ...
  def refint2 (f : RefInt => Unit) : (Int, Int, Int) = {
    ...
    //Call the field last so that all three instances update after third instance is created
    val x1 : Int = r1.get
    val x2 : Int = r2.get
    val x3 : Int = r3.get
    (x1, x2, x3)
  ...
  def refint5 (xs : List[RefInt]) : Unit = {
  	...
  	//Note: Call-By-Reference is visible to the caller while Call-By-Value is not.
  	//So as long as I can manipulate the reference, the caller can see the changes.
  	for (x <- xs) {
  		x.set(3);
  	}

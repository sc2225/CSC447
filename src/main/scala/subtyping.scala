...
object subtyping {
  ...
  // I.e., if the increment method is called 3 times on an instance, and the decrement method is called 2 times on the same
  // instance, then it should store 5  (somewhere other than the existing field n).
  ...
  def observeCounter (f : Counter => Unit) : Int = {
    ...
    class myCounter extends Counter {
    	private var m = 0;
    	override def decrement () = {m = m + 1}
    	override def increment () = { m = m + 1 }
    	override def get () : Int = m
    }
	 val x = new myCounter();
	 f(x);
	 x.get();
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    // TODO: Provide definition here.
    class myObserveCounter extends Counter {
    	private var m = 0;
    	override def decrement () = {m = m + 1}
    	override def increment () = { m = m + 1 }
    	override def get () : Int = m
    ...
    val y = new myObserveCounter();
    val z = new myObserveCounter();
    val q = new myObserveCounter();
    f(List(y,z,q));

 	 myList = List(y.get(), z.get(), q.get());
 	 myList;
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    // TODO: Provide definition here.
    class myObserveArray extends Counter {
    	private var m = 0;
    	override def decrement () = {m = m + 1}
    	override def increment () = { m = m + 1 }
    	override def get () : Int = m
    ...
    var myArrayList : Array[Int] = new Array[Int] (3);
    val a = new myObserveArray();
    val b = new myObserveArray();
    val c = new myObserveArray();
    f(Array(a,b,c));

 	 myArrayList = Array(a.get(), b.get(), c.get());
 	 myArrayList;
   // List (-1, -1, -1).toArray
  }
}

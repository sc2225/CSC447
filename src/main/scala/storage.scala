...
object storage {
  ...
  val getAndSet : Int => (() => Int, Int => Unit) = {
    ...
    (n : Int) => {
    	var aux:Int = n; 
    	val get : () => Int = () => aux
    	val set: Int => Unit = (a : Int) => aux = a
    	(get, set) //Here, we return (get , set) so that it matches the return type declared. it also calls
    	//these methods so they match
     }
  }
  ...
  // "getAndSet" above (with the exception noted next). 
  // The first component of the pair is a function that, when invoked, returns the total number of times 
  //that a "set" call has been made.
  ...
  val getAndSetSpy : () => (() => Int, Int => (() => Int, Int => Unit)) = {
    var count = 0 
  	...
  	//first function in pair () => Int
  	var counter: () => Int = () => count

  	//second function in pair Int => (() => Int, Int => Unit)). This is going to equal the getSet function
  	var getSetAux: Int => (() => Int, Int => Unit) = getSet;
  	
  	//Call both the counter and getSet to return a pair of functions
  	(counter, getSetAux)

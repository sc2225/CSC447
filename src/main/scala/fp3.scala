...
object fp3 {
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    ...
    def countAux [X] (xxs:List[X], element:X, counter:Int) : List[(Int, X)] = 
      { xxs match {
        case Nil =>  
            if (element != null) {
              (counter, element) :: Nil
        ...
        case y :: ys => 
           if (y != element) {
            (counter, element) :: countAux(ys, y, 1)
            } else { 
              countAux(ys, y, counter+1)      
            }
      }

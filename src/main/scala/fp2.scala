...
xs match{
 ...
 //  xs.foldLeft("")( _ + _ + term) 
 foldLeft(xs, "", (x:String, result: String) => x + result + term)
  }
  ...
  def firstNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    ...
    xs match {
    	...
    	case y::ys if(y >= a) => return y
    	case y::ys => firstNumGreaterThan(a,ys)
    }
  ...
  //auxillary function to keep count of the index == it returns the current index
  def countAux(a:Int, xs: List[Int], index : Int) : Int = {
	  xs match {
	    	case Nil => throw new java.util.NoSuchElementException()
	    	case y::ys if(y >= a) => return index
	    	case y::ys => countAux(a, ys, index + 1)
	    }
	}
}
